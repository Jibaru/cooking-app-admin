...
export default {
  ...
  computed: {
    ...
    submitButtonColor() {
      return this.isEditingRecipeType ? "success" : "primary";
    },
    submitButtonText() {
      return this.isEditingRecipeType
        ? "Modificar Tipo de Receta"
  ...
  methods: {
    ...
    submitForm() {
      ...
      const eventName = this.isEditingRecipeType
        ? "submit-edit"
        : "submit-create";

      this.$emit(eventName, {
        ...(this.isEditingRecipeType && {
          recipeTypeId: this.currentId,
        }),
        name: this.name.val,
